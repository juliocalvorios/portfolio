 ...
 */
import { Bug, FlaskConical, Puzzle } from 'lucide-react'
...
// Daily Puzzle Ad - clickeable trigger for Wordle game
export function PuzzleAd({ className = '', onClick }) {
  return (
    <button
      onClick={onClick}
      className={`group relative bg-paper overflow-hidden transition-all duration-300 w-[175px] cursor-pointer text-left ${className}`}
    >
      {/* Outer frame - double border effect */}
      <div className="absolute inset-0 border-2 border-neutral-800 group-hover:border-amber-700 transition-colors duration-300" />
      <div className="absolute inset-[3px] border border-neutral-400 group-hover:border-amber-400 transition-colors duration-300" />

      {/* Corner dots */}
      <span className="absolute top-1.5 left-1.5 w-1 h-1 rounded-full bg-neutral-400 group-hover:bg-amber-600 transition-colors duration-300" />
      <span className="absolute top-1.5 right-1.5 w-1 h-1 rounded-full bg-neutral-400 group-hover:bg-amber-600 transition-colors duration-300" />
      <span className="absolute bottom-1.5 left-1.5 w-1 h-1 rounded-full bg-neutral-400 group-hover:bg-amber-600 transition-colors duration-300" />
      <span className="absolute bottom-1.5 right-1.5 w-1 h-1 rounded-full bg-neutral-400 group-hover:bg-amber-600 transition-colors duration-300" />

      {/* Inner content */}
      <div className="relative m-2 p-2.5 text-center">
        {/* PUZZLE header */}
        <h4
          className="text-xl font-black tracking-wider group-hover:text-amber-700 transition-colors duration-300 mb-0.5"
          style={{ fontFamily: "'Playfair Display', Georgia, serif" }}
        >
          PUZZLE
        </h4>
        <p className="text-[9px] tracking-widest text-neutral-500 group-hover:text-amber-500 transition-colors duration-300 mb-2">
          TODAY'S CHALLENGE
        </p>

        {/* Puzzle icon */}
        <div className="mx-auto w-14 h-14 border border-neutral-800 bg-neutral-100 group-hover:bg-amber-50 group-hover:border-amber-700 transition-all duration-300 mb-2 flex items-center justify-center">
          <Puzzle className="w-7 h-7 text-neutral-300 group-hover:text-amber-500 transition-colors duration-300" />
        </div>

        {/* Game name */}
        <p className="text-[10px] italic text-neutral-500 group-hover:text-amber-600 transition-colors duration-300">
          The Daily
        </p>
        <p className="text-xs font-bold font-serif group-hover:text-amber-800 transition-colors duration-300 mb-1.5">
          Word Game
        </p>

        {/* Description */}
        <div className="text-[10px] text-neutral-500 group-hover:text-amber-600 transition-colors duration-300 mb-2 space-y-0.5">
          <p>★ Get to know Julio</p>
          <p>★ 6 tries to guess</p>
        </div>

        {/* CTA */}
        <div className="border-t border-neutral-300 group-hover:border-amber-300 pt-1.5 transition-colors duration-300">
          <p className="text-[9px] tracking-widest text-neutral-400 group-hover:text-amber-500 transition-colors duration-300">
            CLICK TO
          </p>
          <p className="text-sm font-black font-serif group-hover:text-amber-700 transition-colors duration-300">
            Play Now
          </p>
        </div>
      </div>
      ...
      {/* "PLAY!" stamp on hover */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-30 transition-all duration-500 rotate-[-15deg] scale-0 group-hover:scale-100">
        <div className="border-2 border-amber-600 rounded px-2 py-1">
          <span className="text-base font-black text-amber-600 tracking-wider">PLAY!</span>
        </div>
      </div>
    </button>
  )
